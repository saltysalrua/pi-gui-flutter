...
`provider-switch.ts` resolves image input for all registered models in one place. Since 2.0, capability inference is shared with models.dev metadata in `catalog.mjs`; explicit profile settings still win and this feature never rewrites model entries.

## Resolution

1. An explicit per-model `input` wins, including `["text"]`.
2. A successful, unexpired image probe for the configured route wins next.
3. Otherwise use pi's installed built-in model catalog, then the models.dev catalog cached by `catalog.mjs`. Exact IDs are preferred; namespace prefixes and `【group】` display decorations may be removed for lookup. Conflicting declarations remain unresolved. No model-family/version wildcard is treated as capability evidence.
...
`provider-switch/inputs.ts` owns lookup, the challenge image and cache. `provider-switch.ts` owns profile resolution, real authenticated requests, and applying the result to the still-selected model. A successful probe re-registers the provider; it never changes which model is selected.
