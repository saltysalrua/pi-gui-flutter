...
- 输入卡片、设置分组、模型浮层、弹窗、通知，以及其他使用 `AppCard` 的有底色卡片一起跟随，包括用户消息和带框代码 / Diff。原有背景颜色、圆角、描边、阴影、尺寸和布局不变；正文、代码、图片和操作控件不被模糊或整体淡化。
...
4. 嵌套卡片通过私有 `_CardBackdropScope` 复用祖先模糊，只绘制自身底色，避免反复采样同一背景。无材质、100% 或回退状态下 `BackdropFilter.enabled` 为 false；完全透明的背景和 `glass: false` 显式退出材质，但保留原本的透明语义。
5. 只有原生状态为 `active` 且没有高对比度时才启用卡片效果。系统关闭透明、节电、缺失通道或平台不支持时立即恢复原本实色，保存的选项不变。没有修改原生 C++，已具备毛玻璃通道的 GUI 可热重载使用；旧 Runner 仍需下次启动新版本。
