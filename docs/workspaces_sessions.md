...
- 列表靠近底部「账号额度」入口时使用 [24px 渐变淡出](scroll_edge_fade.md)，滚到最底部恢复清晰；固定入口不参与淡出，不叠加毛玻璃底色。
...
需要 PATH 中的 Node.js、npm 安装的 Pi；Git 功能需要 Git。`PiWorkspaceTransport` 携带八个脚本：`workspace_rpc.mjs`、`workspace_manager.mjs`、`workspace_browser.mjs`、`gui_tool_diff.mjs`、`gui_image_upload.mjs`、`gui_packages.mjs`、`gui_history.mjs`、`gui_provider_profiles.mjs`，解包到同一临时目录。上传图片由适配层复用 Pi 公开缩放函数预处理，见 [图片与附件](images_and_links.md)；插件市场 / 插件管理见 [插件页文档](plugin_packages.md)。关闭应用只清理它自己的进程树和脚本。
