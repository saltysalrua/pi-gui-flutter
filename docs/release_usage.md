...
3. 运行环境需要 Windows x64、PATH 中可用的 Node.js，以及已安装并配置好模型的 Pi：
   ...
   在 Pi 中完成模型和凭据配置后，再打开 GUI。项目当前验证的 Pi 版本为 `0.85.1`；ZIP 不内置 Node.js、Pi、模型凭据或开发者配置。
...
ZIP 旁的 `.zip.sha256` 记录 SHA-256。下载后可运行：
...
关闭旧版后，将新 ZIP 解压到新的目录再运行，不要覆盖仍在运行的 EXE。应用外观配置与附件缓存使用 GUI 自有的系统应用数据目录，Pi 会话与模型配置由 Pi 管理；“便携版”仅表示无需安装器，并不表示用户数据全部写在解压目录。
